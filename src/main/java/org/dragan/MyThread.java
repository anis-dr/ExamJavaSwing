...
public class MyThread extends Thread {
    ...
    public void histogramme(int numberOfPeople) {
        int femaleCount = ml.getModel().getFemaleCount();
        int maleCount = ml.getModel().getMaleCount();

        int maleHeight = 0;
        int femaleHeight = 0;
        if (numberOfPeople != 0) {
            femaleHeight = 240 * femaleCount / numberOfPeople;
            maleHeight = 240 * maleCount / numberOfPeople;
        }
